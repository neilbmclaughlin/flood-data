...
class Station {
  ...
  async saveToDb (stations) {
    ...
    // Update database load timestamp
    await this.db.query(queries.stationLoadTimestamp, [Math.round((new Date()).getTime() / 1000)])

    // Update station ta 8km dataset for new stations
    await this.db.query(queries.updateStationTa8km)
  }
