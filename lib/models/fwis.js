...
class Fwis {
  ...
  async save (warnings, timestamp) {
    ...
    warnings.map(warning => {
      return {
        fwa_code: warning.$.fwacode,
        fwa_key: warning.$.fwakey,
        description: warning.$.description,
        region: warning.$.region,
        area: warning.$.area,
        tidal: warning.$.tidal,
        severity: warning.$.severity,
        severity_value: warning.$.severityvalue,
        warning_key: warning.$.warningkey,
        time_raised: moment.tz(warning.$.timeraised, 'DD MM YYYY HH mm', 'Europe/London').toJSON(), // Note: fwis.xml supplies datetimes in locale format (Europe/London), with no utc offset
        severity_changed: moment.tz(warning.$.severity_changed, 'DD MM YYYY HH mm', 'Europe/London').toJSON(),
        rim_changed: moment.tz(warning.$.rim_changed, 'DD MM YYYY HH mm', 'Europe/London').toJSON(),
        rim: warning.rim_english[0]
      }
    })
